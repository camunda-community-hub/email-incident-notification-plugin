...
Назначение этого плагина -- оповещать пользователей об инцидентах в процессах Камунды по электронной почте.

## Как он работает?

При возникновении инцидента, его данные сохраняются в связном списке (см. метод `BufferingIncidentHandler.handleIncident`).
...
Адреса получателей задаются в переменных процесса в файлах BPMN. Вы можете открыть файл [sample-process.bpmn](docs/sample-process.bpmn) в качестве примера.
...
Если `intervalMs` равен одной минуте (60000 миллисекунд), сообщения отправляются максимум раз в минуту (при наличии инцидентов, о которых необходимо оповестить). То есть, если у вас процесс, в котором инциденты возникают каждую секунду, электронные письма будут отправляться каждую минуту.

Если `intervalMs` установлен в значение 5 минут (300000 миллисекунд) и инциденты возникают каждую секунду, электронные письма будут отправляться каждые пять минут.

Количество писем, отправляемых каждый раз, зависит -- кроме значения `intervalMs` и наличия новых инцидентов -- от количества пар адрес/CC-адрес.
...
Допустим,
   ...
   * `intervalMs` установлен в значение 5 минут (300000 миллисекунд).

В этом случае каждые пять минут по адресу `bob@yourcompany.com` будет отправляться одно письмо.

А теперь представьте, что в одних процессах получателем писем об инцидентах был установлен `bob@yourcompany.com`, а в других -- `alice@yourcompany.com`.

При прочих равных это значит, что каждые пять минут будут отправляться максимум 2 письма (один по адресу `bob@yourcompany.com`, второй по адресу `alice@yourcompany.com`).
...
Тема писем для сообщений об инцидентах.
